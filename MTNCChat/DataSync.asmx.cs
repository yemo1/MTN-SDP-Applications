...
namespace MTNCChat
{
    ...
    public class DataSync : System.Web.Services.WebService
    {
        ...
        public syncOrderRelationResponse syncOrderRelation([System.Xml.Serialization.XmlElementAttribute("syncOrderRelation", Namespace = "http://www.csapi.org/schema/parlayx/data/sync/v1_0/local")] syncOrderRelation syncOrderRelation1)
        {
            ...
            SyncOrderRelation orderRequest = new MTNCChat.SyncOrderRelation();
                    ...
                    if (product != null && product.ID > 0)
                    {
                        ...
                        if (product.IsActive.GetValueOrDefault() == 1)
                        {
                            if (product.IsSub.GetValueOrDefault() == 1)
                            {
                                ...
                                //    & DayDiff(UTC(a.UpdateTime), UTC(syncOrderRelation1.updateTime)) < product.DaysValid.GetValueOrDefault()) == false)
                                //{
                                //    if (syncOrderRelation1.updateType == 1 | (db.SyncOrderRelations.AsEnumerable().Any(a => a.UserID == syncOrderRelation1.userID.ID & a.UserType == syncOrderRelation1.userID.type & a.Passed == 1
                                //        & a.ProductID == syncOrderRelation1.productID & a.ServiceID == syncOrderRelation1.serviceID //& a.UpdateType == syncOrderRelation1.updateType 
            ...
            try
            {
                ...
                orderResponse.ResultInterpretation = (ret.result == "0") ? "Sucess" : ((db.SyncOrderErrors.AsEnumerable().FirstOrDefault(f => f.Code == ret.result) != null) ?
                    db.SyncOrderErrors.AsEnumerable().FirstOrDefault(f => f.Code == ret.result).Cause : "Unknown SDP Code");
        ...
        private DateTime UTC(string time)
        {
            DateTime? val = null;
            try
            { val = DateTime.ParseExact(time, AppUtil.TimeFormat, System.Globalization.CultureInfo.InvariantCulture); }
            catch
            { }
